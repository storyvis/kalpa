...
impl ProviderFactory {
    ...
    ///
    /// Uses a refreshable service-account-backed provider so long-running
    /// processes do not hit `401 invalid_token` after the ~1h access-token TTL.
    pub async fn vertex(config: &KalpaConfig) -> KalpaResult<VertexProvider> {
        ...
        VertexProvider::from_service_account_file(service_account_path, location, gcs_bucket)
            .await
